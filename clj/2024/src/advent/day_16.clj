...
(comment
  (let [vacant? #(= \. %)
    ...
    (let [cheapest-path (first (sort-by (fn [path] (-> path last :score)) result))
          ...
          coords-part-of-some-shortest-path (->> shortest-paths
                                                 (mapcat (fn [path] (map #(:coord %) path)))
                                                 (into #{}))
          ]

      #_(prn "SHORTEST PATHS " (count shortest-paths))
      #_(doseq [x result]
         (prn "COST " (last x))

        (->> x
             (reduce (fn [acc {:keys [coord value]}]
                       (assoc acc coord value)) xy-to-char)
             (util/reify-map)
             (util/print-map!))
      ...
      #_(prn cheapest-path)
      #_(util/print-map! (util/reify-map (reduce (fn [acc {:keys [coord value]}]
                                                   (assoc acc coord value)
                                                   ) xy-to-char cheapest-path)))
      #_(util/print-map! (util/reify-map (reduce (fn [acc coord]
                                                   (assoc acc coord \O)
                                                   ) xy-to-char coords-part-of-some-shortest-path)))
