...
(defn is-uphill? [from to xy-to-num]
  (= (get xy-to-num from)
...
(defn day-10-star-1
  ([path]
   (day-10-star-1 path #{}))                                ;; Count unique by default
  ([path initial-reached-9s]
...
(defn day-10-star-2 [path]
  (day-10-star-1 path [])                                   ;; Only difference is we want to count ALL paths
  )
